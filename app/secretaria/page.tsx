...
export default function SecretariaDashboard() {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans" style={{ background: 'var(--bg)' }}>
      ...
      {/* ── SIDEBAR ── */}
      <aside className={`
        fixed md:static z-40 h-full w-[215px] flex flex-col flex-shrink-0
        transition-transform duration-300 ease-in-out
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `} style={{ background: 'var(--sidebar)', borderRight: '1px solid var(--card-border)' }}>
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 px-2 py-1.5"
